...
impl PrcRuntimeSession {
    pub fn from_source<S: AppSource>(
        ...
        runtime_ctx.fonts = crate::palm::font::load_nfnt_fonts(&runtime_ctx.resources);
        ...
        runtime_ctx.databases.push(runtime::RuntimeDatabase {
            ...
            is_resource_db: matches!(info.kind, crate::palm::PrcDbKind::Resource),
        ...
        });
        {
            let mut parsed_forms =
                crate::palm::form_preview::parse_form_previews_from_resource_blobs(
                    &runtime_ctx.resources,
                );
            if parsed_forms.iter().all(|f| f.objects.is_empty()) && !runtime_ctx.prc_image.is_empty()
            {
                parsed_forms = crate::palm::form_preview::parse_form_previews(&runtime_ctx.prc_image);
            }
                ...
                })
            {
                for (idx, obj) in form.objects.iter().enumerate() {
                    let (object_id, kind) = match obj {
                        crate::palm::form_preview::FormPreviewObject::Field { id, .. } => {
                            (*id, runtime::RuntimeFormObjectKind::Field)
                        }
                        crate::palm::form_preview::FormPreviewObject::Button { id, .. } => {
                            (*id, runtime::RuntimeFormObjectKind::Other)
                        }
                        crate::palm::form_preview::FormPreviewObject::Table { id, .. } => {
