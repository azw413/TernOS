...
cd "$ROOT/m5paper-rust"
cargo +esp build --release

export TERN_RUST_LIB_PATH="$ROOT/target/xtensa-esp32-espidf/release/libtern_m5paper_rust.a"
...
if [[ "$MONITOR" == "1" ]]; then
  ...
  echo "To monitor:"
  echo "  cd $ROOT/m5paper-rust/espidf && $PYENV $IDFPY -p $PORT monitor"
fi
