...
unsafe extern "C" {
    ...
    fn tern_m5paper_backend_battery_read(out_status: *mut BatteryStatus) -> Status;
    fn tern_m5paper_backend_sleep(deep: bool) -> Status;
...
pub fn epd_update_region(x: u16, y: u16, width: u16, height: u16, data: &[u8], mode: UpdateMode) -> Status {
    unsafe {
        tern_m5paper_backend_epd_update_region(x, y, width, height, data.as_ptr(), data.len() as u32, mode)
    }
}
...
pub fn battery_read() -> Result<BatteryStatus, Status> {
    let mut status_out = BatteryStatus::default();
    let status = unsafe { tern_m5paper_backend_battery_read(&mut status_out) };
    if status == Status::Ok {
        Ok(status_out)
    } else {
        Err(status)
...
pub fn sleep(deep: bool) -> Status {
    unsafe { tern_m5paper_backend_sleep(deep) }
}
...
pub fn log_storage_entry(entry: &StorageEntry) {
    ...
    unsafe {
        ets_printf(
            b"m5paper: entry type=%s name=%s size=%u\n\0".as_ptr(),
            type_str.as_ptr(),
            entry.name.as_ptr(),
            entry.size,
        );
